  ...
  <div v-if="activity && activity.steps && activity.steps.length" class="my-1">
    ...
    <div v-if="!activity.done" class="rounded-lg border border-gray-200 bg-slate-50/70 px-3 py-2">
      <div class="flex items-center justify-between mb-1.5">
        <div class="inline-flex items-center gap-1.5 text-[11px] px-2 py-0.5 rounded-full bg-indigo-50 text-indigo-600">
    ...
    <div v-else class="rounded-lg border border-gray-200 bg-slate-50/70">
      <button type="button" @click="expanded = !expanded"
        ...
        <span>{{ anyError ? 'Completed with issues' : 'Done' }} · {{ summary.count }} step{{ summary.count === 1 ? '' : 's' }}<span v-if="summary.seconds"> · {{ summary.seconds.toFixed(1) }}s</span><span v-if="liveTokens"> · {{ fmtTokens(liveTokens.total) }} tokens</span></span>
